...
async def test_get_operations():
    ...
    async with client_session(mcp._mcp_server) as client:
        ...
        assert timeline_result["status"] == "success", f"Failed to get timeline: {timeline_result.get('message')}"
        ...
        assert isinstance(timeline_data, dict), f"Timeline data should be dict, got {type(timeline_data)}"
        ...
        assert len(feed_items) > 0, "No posts in timeline"
        ...
        assert post_result["status"] == "success", f"Failed to get post: {post_result.get('message')}"
        ...
        assert posts_result["status"] == "success", f"Failed to get posts: {posts_result.get('message')}"
        ...
        print(f"   - get_post successfully retrieved post")
