...
async def test_follow_unfollow_operations():
    ...
    async with client_session(mcp._mcp_server) as client:
        ...
        if follow_result["status"] == "error" and "already following" in follow_result.get("message", "").lower():
        ...
        assert follow_result["status"] == "success", f"Failed to follow user: {follow_result.get('message')}"
        ...
        assert unfollow_result["status"] == "success", f"Failed to unfollow user: {unfollow_result.get('message')}"
        ...
        print(f"   - Verified user is no longer in follows list")
